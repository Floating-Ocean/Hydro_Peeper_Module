...
public class SubmissionTool {
    ...
    public static List<SubmissionData> fetchData(boolean isYesterday) throws Throwable {
        ...
        while (true) {
            final String url = "https://fjnuacm.top/d/junior/record?page=" + (i++);
            ...
            for (var each : data.getElementsByTag("tr")) {
                ...
                submissionDataList.add(new SubmissionData(submitterName, submitterId, score, VerdictType.searchVerdict(verdict), problemName, at));
            }
            if (eof) break;
        }
    ...
    public static Pair<Pair<Integer, Integer>, List<SimpleRankableRecord>> fetchVerdictRank(List<SubmissionData> submissionDataList, VerdictType type) {
        ...
        for (var each : submissionDataList) {
            ...
            count++;
            int pre = topCount.getOrDefault(new UserData(each.user(), each.id()), 0);
            topCount.put(new UserData(each.user(), each.id()), pre + 1);
        }
        List<SimpleRankableRecord> result = new ArrayList<>();
        for (var each : topCount.keySet()) {
            result.add(new SimpleRankableRecord(each.name(), topCount.get(each)));
        }
    ...
    public static CounterData findMostPopularProblem(List<SubmissionData> submissionDataList) {
        ...
        for (var each : submissionDataList) {
            ...
            submissionMap.get(each.problemName()).add(new UserData(each.user(), each.id()));
        }
     ...
     */
    public static Pair<Long, Pair<String, String>> getFirstACAttempt(List<SubmissionData> submissionData) {
